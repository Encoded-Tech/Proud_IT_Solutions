...
function QuotationPage({
  ...
  return (
    <article
      ...
      data-quotation-export-page="true"
      style={{
        width: "210mm",
        height: "297mm",
        minHeight: "297mm",
    ...
    >
      ...
      <div className="page-number q-text-700">
        Page {page.pageNumber} of {page.totalPages}
      </div>

      <div
        className={`quotation-content q-text-900 ${
          page.isFinalPage ? "final-page" : "non-final-page"
        }`}
      >
        <div>
          <div className="flex items-start justify-between gap-4">
            <div className="client-details max-w-[115mm]">
              <h1 className="quotation-title q-text-900 text-center text-[15px] font-black uppercase tracking-[0.22em]">
                {page.isFirstPage ? "Quotation" : "Quotation Continued"}
              </h1>

              {page.isFirstPage ? (
                <div className="q-text-800 mt-3 space-y-1 text-[10.5px] leading-5">
                  <p>
                    <span className="q-text-800 font-bold">Client Name:</span>{" "}
                    {draft.party.clientName}
                  </p>
                  {draft.party.clientCompanyName ? (
                    <p>
                      <span className="q-text-800 font-bold">Company:</span>{" "}
                      {draft.party.clientCompanyName}
                    </p>
                  ) : null}
                  {draft.party.clientAddress ? (
                    <p>
                      <span className="q-text-800 font-bold">Address:</span>{" "}
                      {draft.party.clientAddress}
                    </p>
                  ) : null}
                  {draft.party.clientContact ? (
                    <p>
                      <span className="q-text-800 font-bold">Phone / Email:</span>{" "}
                      {draft.party.clientContact}
                    </p>
                  ) : null}
                </div>
...
const QuotationPreview = forwardRef<HTMLDivElement, QuotationPreviewProps>(function QuotationPreview(
  { draft },
  ref
) {
  ...
  return (
    <div className="space-y-4">
      <style jsx global>{`
        ...
        .quotation-content.non-final-page {
          bottom: 28mm;
        }

        .quotation-content.final-page {
          bottom: 22mm;
        }
      ...
      <div ref={ref} className="quotation-export-root" data-quotation-export="true">
        {pages.map((page) => (
          <QuotationPage
            key={page.pageNumber}
            draft={draft}
